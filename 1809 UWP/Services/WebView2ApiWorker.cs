    ...
    public class WebView2ApiWorker : IApiWorker
    {
        ...
        public Task InitializeAsync(string baseUrl = null)
        {
            ...
            _ = CoreApplication.MainView.CoreWindow.Dispatcher.RunAsync(CoreDispatcherPriority.Normal, async () =>
            {
                try
                {
                    if (App.UIHost == null) throw new InvalidOperationException("App.UIHost is not available for WebView2 worker.");
                    WebView = new WebView2();
                    App.UIHost.Children.Add(WebView);
                    await WebView.EnsureCoreWebView2Async();
                    if (!string.IsNullOrEmpty(baseUrl)) WebView.CoreWebView2.Navigate(baseUrl);
                    tcs.SetResult(true);
                }
        ...
        public async Task<byte[]> GetRawBytesFromUrlAsync(string url)
        {
            ...
            else if (new[] { ".png", ".jpg", ".jpeg", ".gif", ".webp", ".bmp" }.Contains(extension))
            {
        ...
        private Task<byte[]> GetSvgBytesAsync(string url)
        {
            ...
            _ = CoreApplication.MainView.CoreWindow.Dispatcher.RunAsync(CoreDispatcherPriority.Normal, async () => {
                ...
                try
                {
                    ...
                    if (this.WebView?.CoreWebView2 != null) await CopyCookiesInternalAsync(this.WebView.CoreWebView2, tempWorker.CoreWebView2);
                ...
                finally { App.UIHost.Children.Remove(tempWorker); tempWorker.Close(); }
            });
            return tcs.Task;
        }
        ...
        private Task<byte[]> GetRasterImageBytesAsync(string url)
        {
            ...
            _ = CoreApplication.MainView.CoreWindow.Dispatcher.RunAsync(CoreDispatcherPriority.Normal, async () => {
                ...
                try
                {
                    ...
                    if (!await navTcs.Task) throw new Exception($"Image navigation failed for {url}");
                    const string script = @"(function() { const img = document.querySelector('img'); if (img && img.naturalWidth > 0) { const canvas = document.createElement('canvas'); canvas.width = img.naturalWidth; canvas.height = img.naturalHeight; const ctx = canvas.getContext('2d'); ctx.drawImage(img, 0, 0); return canvas.toDataURL('image/png').split(',')[1]; } return null; })();";
                    string scriptResult = await tempWorker.CoreWebView2.ExecuteScriptAsync(script);
                    if (string.IsNullOrEmpty(scriptResult) || scriptResult == "null") throw new Exception("Failed to get Base64 data from rasterization script.");
        ...
        private Task<byte[]> DownloadFileAsBytesAsync(string url, string extension)
        {
            ...
            _ = CoreApplication.MainView.CoreWindow.Dispatcher.RunAsync(CoreDispatcherPriority.Normal, async () =>
            {
                ...
                try
                {
                    ...
                    if (!await navTcs.Task) throw new Exception($"Navigation to base URL '{AppSettings.BaseUrl}' failed, preventing download context setup.");
        ...
        {
            if (worker?.CoreWebView2 == null) throw new InvalidOperationException("CoreWebView2 is not initialized.");
            var navCompleteTcs = new TaskCompletionSource<bool>();
            TypedEventHandler<CoreWebView2, CoreWebView2NavigationCompletedEventArgs> navHandler = null;
            navHandler = (s, e) => { s.NavigationCompleted -= navHandler; navCompleteTcs.TrySetResult(e.IsSuccess); };
        ...
        {
            return GetContentFromUrlCore(url, (fullHtml) => !string.IsNullOrEmpty(fullHtml) ? fullHtml : null);
        }
        ...
        private async Task CopyCookiesInternalAsync(CoreWebView2 source, CoreWebView2 destination)
        {
            ...
            {
                var newCookie = destination.CookieManager.CreateCookie(cookie.Name, cookie.Value, cookie.Domain, cookie.Path);
        ...
        {
            if (source is WebView2ApiWorker sourceWvWorker)
            {
                if (this.WebView?.CoreWebView2 != null && sourceWvWorker.WebView?.CoreWebView2 != null)
                {
                    await CopyCookiesInternalAsync(sourceWvWorker.WebView.CoreWebView2, this.WebView.CoreWebView2);
                }
            }
        }
        ...
        private async Task<string> GetContentFromUrlCore(string url, Func<string, string> validationLogic)
        {
            ...
            {
                if (WebView?.CoreWebView2 == null) throw new InvalidOperationException("CoreWebView2 is not initialized.");
                var navCompleteTcs = new TaskCompletionSource<bool>();
                TypedEventHandler<CoreWebView2, CoreWebView2NavigationCompletedEventArgs> navHandler = null;
                navHandler = (s, e) => { s.NavigationCompleted -= navHandler; navCompleteTcs.TrySetResult(e.IsSuccess); };
                ...
                while (stopwatch.Elapsed.TotalSeconds < 15)
                {
                    ...
                    string scriptResult = await WebView.CoreWebView2.ExecuteScriptAsync("document.documentElement.outerHTML");
        ...
        {
            return GetContentFromUrlCore(url, (fullHtml) =>
            {
        ...
        public async Task<string> PostAndGetJsonFromApiAsync(string url, Dictionary<string, string> postData)
        {
            return await CoreApplication.MainView.CoreWindow.Dispatcher.RunTaskAsync(async () =>
            {
                if (WebView?.CoreWebView2 == null) throw new InvalidOperationException("CoreWebView2 could not be initialized for POST.");
                ...
                using (var stream = new InMemoryRandomAccessStream())
                {
                    ...
                    stream.Seek(0);
                    var request = WebView.CoreWebView2.Environment.CreateWebResourceRequest(url, "POST", stream, "Content-Type: application/x-www-form-urlencoded");
