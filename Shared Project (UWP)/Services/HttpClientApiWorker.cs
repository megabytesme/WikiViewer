...
namespace Shared_Code
{
    public class HttpClientApiWorker : IApiWorker
    {
        ...
        private string ExtractJsonFromHtml(string responseText)
        {
            ...
            {
                Console.WriteLine("[ERROR] Failed to parse HTML for JSON extraction.");
            }
        ...
        private async Task<byte[]> MakeGatekeeperRequest(object payload)
        {
            ...
            Console.WriteLine($"[SIZE] Outgoing request payload size: {jsonPayload.Length} bytes");

            HttpResponseMessage response;
            try
            {
                response = await _gatekeeperClient.PostAsync(GatekeeperEndpoint + "/v1/request", content);
            }
            catch (Exception ex)
            {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                try
                {
                    ...
                    throw new Exception($"Proxy returned an error: {response.StatusCode} - {errorMessage} {errorDetails}");
                }
                catch
                {
                    throw new Exception($"Proxy returned a non-success status code: {response.StatusCode} - {errorContent}");
                }
            }
            ...
            try
            {
                ...
                Console.WriteLine($"[SIZE] Incoming response body size: {bytes.Length} bytes");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[ERROR] Failed to read response body. Exception: {ex.Message}");
                throw new Exception("Failed to read the response body.", ex);
            }

            if (bytes.Length > 2 && bytes[0] == 0x1F && bytes[1] == 0x8B)
            {
                Console.WriteLine("[GZIP] Decompressing GZIP-encoded response.");
                ...
                using (var resultStream = new System.IO.MemoryStream())
                {
                    ...
                    Console.WriteLine($"[GZIP] Decompressed size: {bytes.Length} bytes");
                }
            }
