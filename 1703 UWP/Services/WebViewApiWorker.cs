...
namespace _1703_UWP.Services
{
    public class WebViewApiWorker : IApiWorker
    {
        ...
        private static HttpClient _cookieAwareHttpClient;

        public Task InitializeAsync(string baseUrl = null)
        {
            Debug.WriteLine("[WebViewApiWorker] InitializeAsync called.");
            var filter = new HttpBaseProtocolFilter();
            filter.CookieUsageBehavior = HttpCookieUsageBehavior.Default;
            _cookieAwareHttpClient = new HttpClient(filter);
            ...
            _ = CoreApplication.MainView.CoreWindow.Dispatcher.RunAsync(CoreDispatcherPriority.Normal, () =>
            {
                try
                {
                    if (WikiViewer.Shared.Uwp.App.UIHost == null) throw new InvalidOperationException("App.UIHost is not available for WebView worker.");
                    WebView = new WebView();
                    ...
                    navHandler = (s, e) => {
                        Debug.WriteLine($"[WebViewApiWorker] Initial navigation completed. Success: {e.IsSuccess}");
                        WebView.NavigationCompleted -= navHandler;
                        if (!tcs.Task.IsCompleted) tcs.SetResult(e.IsSuccess);
                    };
                    WebView.NavigationCompleted += navHandler;
                    var urlToNavigate = new Uri(baseUrl ?? AppSettings.BaseUrl);
                    Debug.WriteLine($"[WebViewApiWorker] Performing initial navigation to: {urlToNavigate}");
                    WebView.Navigate(urlToNavigate);
                }
                catch (Exception ex)
                {
                    Debug.WriteLine($"[WebViewApiWorker] CRITICAL FAILURE during InitializeAsync: {ex.Message}");
        ...
        {
            return await DispatcherTaskExtensions.RunTaskAsync(CoreApplication.MainView.CoreWindow.Dispatcher, async () =>
            {
                ...
                try
                {
                    ...
                    string fullHtml = await tempWebView.InvokeScriptAsync("eval", new[] { "document.documentElement.outerHTML" });
        ...
        private async Task<string> GetContentFromUrlCore(string url, Func<string, string> validationLogic)
        {
            return await DispatcherTaskExtensions.RunTaskAsync(CoreApplication.MainView.CoreWindow.Dispatcher, async () =>
            {
                if (url == null)
                {
                    Debug.WriteLine("[WebViewApiWorker] CRITICAL: GetContentFromUrlCore received a NULL url.");
                    throw new ArgumentNullException(nameof(url), "URL cannot be null for WebView navigation.");
                }
                ...
                TypedEventHandler<WebView, WebViewNavigationCompletedEventArgs> navHandler = null;
                navHandler = (s, e) => {
                    Debug.WriteLine($"[WebViewApiWorker] -> NavigationCompleted for {url}. Success: {e.IsSuccess}, Status: {e.WebErrorStatus}");
                    WebView.NavigationCompleted -= navHandler;
                    navTcs.TrySetResult(e.IsSuccess);
                };
                WebView.NavigationCompleted += navHandler;
                ...
                while (stopwatch.Elapsed.TotalSeconds < 15)
                {
                    ...
                    Debug.WriteLine($"[WebViewApiWorker] -> Scraping attempt for {url}. HTML length: {fullHtml?.Length ?? 0}");
                    if (string.IsNullOrEmpty(fullHtml)) continue;

                    string extractedContent = validationLogic(fullHtml);
                    if (extractedContent != null)
                    {
                        Debug.WriteLine($"[WebViewApiWorker] -> SUCCESS: Content validated for {url}.");
                        return extractedContent;
                    }
                }
                Debug.WriteLine($"[WebViewApiWorker] -> FAILED: Content validation timed out for {url}.");
            ...
            return await DispatcherTaskExtensions.RunTaskAsync(CoreApplication.MainView.CoreWindow.Dispatcher, async () =>
            {
                ...
                try
                {
                    scriptNotifyHandler = (object sender, NotifyEventArgs args) =>
                    {
                        ...
                        {
                            string base64Data = notification.Substring("DOWNLOAD_SUCCESS:".Length);
                            try
                            {
                                var bytes = Convert.FromBase64String(base64Data);
                                Debug.WriteLine($"[WebViewApiWorker-Bytes] -> SUCCESS: Received {bytes.Length} bytes via ScriptNotify for {url}.");
                                tcs.TrySetResult(bytes);
                            }
                            catch (FormatException ex)
                            {
                                Debug.WriteLine($"[WebViewApiWorker-Bytes] -> FAILED: Invalid Base64 data received. {ex.Message}");
                                tcs.TrySetResult(null);
                            }
                        }
                        else if (notification.StartsWith("DOWNLOAD_ERROR:"))
                        {
                            string errorMessage = notification.Substring("DOWNLOAD_ERROR:".Length);
                            Debug.WriteLine($"[WebViewApiWorker-Bytes] -> FAILED: JS reported an error for {url}: {errorMessage}");
                            tcs.TrySetResult(null);
                        }
                    };
                    tempWebView.ScriptNotify += scriptNotifyHandler;
                    ...
                    tempWebView.NavigationCompleted += navHandler;
                    tempWebView.NavigateToString("<html><head><title>Downloader</title></head><body></body></html>");

                    if (!await navTcs.Task) throw new Exception("Navigation to downloader page failed.");
                ...
                {
                    if (scriptNotifyHandler != null) tempWebView.ScriptNotify -= scriptNotifyHandler;
                    WikiViewer.Shared.Uwp.App.UIHost.Children.Remove(tempWebView);
                    tempWebView = null;
                }
            });
        ...
        public Task CopyApiCookiesFromAsync(IApiWorker source)
        {
            ...
            Debug.WriteLine($"[WebViewApiWorker] CopyApiCookiesFromAsync: Found {sourceCookies.Count} cookies to share.");
