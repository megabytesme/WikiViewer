    ...
    public sealed partial class CreateAccountPage : Page
    {
                    ...
                    foreach (var field in request.Fields)
                    {
                        ...
                        switch (fieldInfo.Type)
                        {
                            ...
                            case "info":
                                ...
                                break;
                            case "password":
                                var passwordBox = new PasswordBox { Header = fieldInfo.Label, Tag = fieldName, PlaceholderText = fieldInfo.Help };
                            ...
                            default:
                                ...
                                {
                                    var textBox = new TextBox { Header = fieldInfo.Label, Tag = fieldName, PlaceholderText = fieldInfo.Help };
        ...
        private TextBlock CreateWikitextBlock(string text)
        {
            ...
            foreach (var part in parts)
            {
                if (part.StartsWith("[[") && part.EndsWith("]]"))
                {
                    ...
                    hyperlink.Click += async (s, ev) => await Windows.System.Launcher.LaunchUriAsync(s.NavigateUri);
        ...
        private string ParseWikitextToHtml(string text)
        {
            ...
            else
            {
                ...
                bodyContent = System.Text.RegularExpressions.Regex.Replace(bodyContent, @"(src|href)=""//", match => $"{match.Groups[1].Value}=\"https://");
            ...
            return $@"
                <html><head><style>
                    body {{ font-family: 'Segoe UI', sans-serif; color: {(Application.Current.RequestedTheme == ApplicationTheme.Dark ? "white" : "black")}; background-color: transparent; margin: 0; padding: 8px; font-size: 14px; text-align: center; display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100vh; box-sizing: border-box; }}
                ...
                </style></head><body><div>{bodyContent}</div></body></html>";
        }
        ...
        private async void CreateButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    var successDialog = new ContentDialog { Title = "Account Created", Content = $"Your account '{result.Username}' was created successfully. You can now log in.", CloseButtonText = "OK" };
