...
namespace _1703_UWP
{
    public sealed partial class FavouritesPage : Page
    {
        ...
        {
            void onAnimationCompleted(object s, object ev)
            {
        ...
        {
            if (_isCachingInProgress) return;
            try
            {
                _isCachingInProgress = true;
                var allFavouriteArticles = FavouritesService.GetFavourites().Where(t => !t.StartsWith("Talk:") && !t.StartsWith("User talk:")).ToList();
                if (!allFavouriteArticles.Any()) return;
                var checkTasks = allFavouriteArticles.Select(async title => new { Title = title, IsCached = await ArticleCacheManager.GetCacheMetadataAsync(title) != null });
                var cacheStatusResults = await Task.WhenAll(checkTasks);
                var titlesToCache = cacheStatusResults.Where(r => !r.IsCached).Select(r => r.Title).ToList();
        ...
        {
            if (fullTitle.StartsWith("Talk:")) return fullTitle.Substring("Talk:".Length);
            if (fullTitle.StartsWith("User talk:")) return fullTitle.Substring("User talk:".Length);
            if (fullTitle.StartsWith("User:")) return fullTitle.Substring("User:".Length);
            return fullTitle;
        }
        ...
        private async Task FindAndSetLeadImage(FavouriteItem item)
        {
            ...
            var allCachedImageNodes = doc.DocumentNode.SelectNodes($"//img[contains(@src, 'ms-local-stream')]");
            if (allCachedImageNodes == null || !allCachedImageNodes.Any()) return;

            string chosenImageUrl = null;
            var nonSvgNode = allCachedImageNodes.FirstOrDefault(node => !node.GetAttributeValue("src", "").EndsWith(".svg", StringComparison.OrdinalIgnoreCase));
            if (nonSvgNode != null)
            {
            ...
            {
                chosenImageUrl = allCachedImageNodes.FirstOrDefault()?.GetAttributeValue("src", null);
            }

            if (!string.IsNullOrEmpty(chosenImageUrl))
            {
                item.ImageUrl = chosenImageUrl;
            }
        }
        ...
        private async void NavigateToArticleIfItExists(string pageTitle)
        {
            ...
            try
            {
                ...
                {
                    var dialog = new ContentDialog { Title = "Page Does Not Exist", Content = $"The page \"{pageTitle.Replace('_', ' ')}\" has not been created yet. Would you like to create it?", PrimaryButtonText = "Create", CloseButtonText = "Cancel" };
                    var result = await dialog.ShowAsync();
                    if (result == ContentDialogResult.Primary)
                    {
                        if (!AuthService.IsLoggedIn)
                        {
                            await new ContentDialog { Title = "Login Required", Content = "You must be logged in to create or edit pages.", CloseButtonText = "OK" }.ShowAsync();
                        }
                        else
                        {
            ...
            {
                await new ContentDialog { Title = "An Error Occurred", Content = $"Could not verify the page's status. Please try again.\n\nError: {ex.Message}", CloseButtonText = "OK" }.ShowAsync();
            }
            finally
            {
                HideLoadingOverlay();
        ...
        private async void DeleteButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            foreach (var item in itemsToDelete)
            {
                ...
                else if (item.IsTalkAvailable) titlesToRemove.Add(item.TalkPageTitle);
            }
